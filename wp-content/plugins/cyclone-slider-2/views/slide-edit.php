	...
	<div class="cs-body">
		<div class="cs-slide-type-bar">
			<select class="cs-slide-type-switcher" name="cycloneslider_metas[<?php echo $i; ?>][type]">
				...
				<option value="video" <?php selected($slide['type'], 'video'); ?>><?php _e('Video', 'cycloneslider'); ?></option>
		...
		<div class="cs-slide-video">
			<div class="cs-video-preview">
				...
				<div class="cs-dialogue-area">

					<div class="cs-video-title" name="cycloneslider_metas[<?php echo $i; ?>][video_title]" <?php echo (empty($slide['video_title'])) ? 'style="display:none"' : '';?>>
						<?php echo $slide['video_title']; ?>
					</div>
					<input class="cs-video-id" name="cycloneslider_metas[<?php echo $i; ?>][video_id]" type="hidden" value="<?php echo esc_attr($slide['video_id']); ?>" />
					<input class="button-secondary cs-video-gallery-show" type="button" value="<?php _e('Get Video', 'cycloneslider'); ?>" />
				</div>

				<div class="cs-dialogue-area">
					<div class="cs-poster-thumb" <?php echo (empty($poster_url)) ? 'style="display:none"' : '';?>>
						<?php if($poster_url): ?>
						<img src="<?php echo esc_url($poster_url); ?>" height="120" width="120" alt="thumb">
						<?php endif; ?>
					</div>

					<input class="cs-poster-id" name="cycloneslider_metas[<?php echo $i; ?>][poster_id]" type="hidden" value="<?php echo esc_attr($slide['poster_id']); ?>" />
					<input class="button-secondary cs-poster-gallery-show" type="button" value="<?php _e('Choose Video Screenshot', 'cycloneslider'); ?>" />
				</div>
			</div>
