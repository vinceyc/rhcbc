...
jQuery(document).ready(function($){
    ...
    (function() {
        ...
        /*** Add video to slide ***/
        $('#cyclone-slides-metabox').on('wpAddVideo', '.cs-video-gallery-show', function(e, attachment_id, attachment_title){
            var current_slide_box, slide_thumb, slide_attachment_id;

            current_slide_box = $(this).parents('.cs-slide');/*** Get current box ***/
            slide_title = current_slide_box.find('.cs-video-title');/*** Find the thumb ***/
            slide_attachment_id = current_slide_box.find('.cs-video-id ');/*** Find the hidden field that will hold the attachment id ***/

            slide_title.html(attachment_title).show();
            slide_attachment_id.val(attachment_id);

        });

        /*** Add poster to Video slide ***/
        $('#cyclone-slides-metabox').on('wpAddPoster', '.cs-poster-gallery-show', function(e, poster_url, attachment_id){
            var current_slide_box, slide_thumb, slide_attachment_id;

            current_slide_box = $(this).parents('.cs-slide');/*** Get current box ***/
            slide_thumb = current_slide_box.find('.cs-poster-thumb');/*** Find the thumb ***/
            slide_attachment_id = current_slide_box.find('.cs-poster-id ');/*** Find the hidden field that will hold the attachment id ***/

            slide_thumb.html('<img src="'+poster_url+'" alt="thumb">').show();
            slide_attachment_id.val(attachment_id);
    ...
    (function() {
        ...
        $(document.body).on('click', '.cs-media-gallery-show', function(e){
            ...
            cyclone_media_frame.open();
        }).on('click', '.cs-video-gallery-show', function(e){
            // Prevent the default action from occuring.
            e.preventDefault();
            ...
            triggering_element = jQuery(this); /* Get current clicked element */
              ...
              // If the frame already exists, re-open it.
                if ( cyclone_video_frame ) {
                    cyclone_video_frame.open();
                    return;
                }

                cyclone_video_frame = wp.media.frames.cyclone_video_frame = wp.media({
                    className: 'media-frame cs-frame',
                    frame: 'select',
                    multiple: false,
                    title: cycloneslider_admin_vars.title,
                ...
                cyclone_video_frame.on('select', function(){
                    var video_attachment, img_url;

                    // Grab our attachment selection and construct a JSON representation of the model.
                    video_attachment = cyclone_video_frame.state().get('selection').first().toJSON();

                    triggering_element.trigger('wpAddVideo', [ video_attachment.id, video_attachment.title]);
                });

                // Now that everything has been set, let's open up the frame.
                cyclone_video_frame.open();
        }).on('click', '.cs-poster-gallery-show', function(e){
            // Prevent the default action from occuring.
            e.preventDefault();
            ...
            cyclone_media_frame.on('select', function(){
                var media_attachment, poster_url;

                // Grab our attachment selection and construct a JSON representation of the model.
                media_attachment = cyclone_media_frame.state().get('selection').first().toJSON();

                poster_url = media_attachment.url;

                triggering_element.trigger('wpAddPoster', [poster_url, media_attachment.id]);
            });
